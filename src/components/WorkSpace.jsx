...
const WorkSpace = () => {
  ...
  const getWorkspace = async (userId) => {
    try {
      ...
      console.log(workSpaceName[0].title)
    } catch (error) {
  ...
  const deleteItem = async (wid) => {
    try {
      await deleteDoc(doc(db, "workspace", wid));
      getWorkspace(user ? user.uid : sessionStorage.getItem("userId"));
    } catch (error) {
    ...
    <div className="flex flex-col lg:flex-row w-full min-h-screen bg-white">
      <Sidebar createitems={
        <OpenModal 
          ...
          headerText="Create your workspace here!" 
          onSubmit={(value) => workspaceCreation(value)} 
        />
      } />
            ...
            {workSpaceName?.length > 0 ? (
              <div className="flex flex-wrap justify-center gap-6 mt-8 w-full">
                {workSpaceName.map((works) => (
                  <div className="flex flex-col justify-center items-center w-64 h-52 p-12 mb-6 bg-mywhitetext border-2 border-slate-200 rounded-md shadow-lg shadow-indigo-200/50 box-border" key={works.id}>
                    <div>
                      <button className='transition ease-in-out delay-150 p-4 rounded-md text-myblue hover:-translate-y-1 hover:scale-110 hover:bg-purple-200 duration-300'>
                        <Link className="workspace-extra-css" to={`/${works.title}/${works.id}`} state={{ title: works.title }}>
                          <h3 className='text-xs font-bold'>{works.title}</h3>
                        </Link>
                      </button>
                    </div>
                    <div className="flex w-full h-full justify-between gap-4 -mb-12 mt-20 pb-2">
                      <p className='text-xs font-medium -ml-8'>{works.formattedDate}</p>
